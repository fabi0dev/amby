  ...
  try {
    ...
    try {
      ...
      suggestions = Array.isArray(parsed)
        ? parsed.filter((s): s is string => typeof s === 'string').map((s) => String(s).trim()).filter(Boolean)
        : []
    } catch {
