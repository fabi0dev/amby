...
export function SearchModal({ open, onOpenChange, container }: SearchModalProps) {
  ...
  return (
    <CommandDialog
      ...
      contentClassName={cn(
        'fixed bg-red-500  left-1/2 top-1/3 z-10 w-[calc(100vw-2rem)] max-w-2xl min-w-[280px] -translate-x-1/2 -translate-y-1/2 flex flex-col rounded-xl border border-border bg-background shadow-2xl overflow-hidden pt-0',
      )}
      ...
      <CommandList className="flex-1 min-h-0 max-h-[min(60vh,400px)] overflow-y-auto overflow-x-hidden px-3 py-2">
        ...
        {!loading && (documents.length > 0 || workspaces.length > 0) && (
          <>
            ...
            {documents.length > 0 && (
              <CommandGroup heading="Documentos" className="pt-1">
                {documents.map((d) => (
                  <CommandItem
                    key={d.id}
                    value={`doc-${d.id}`}
